...
int main(int argc, char* argv[]) {
    ...
    struct stat fileStat;   // Estrutura para armazenar informações do arquivo
    ...
    while ((entry = readdir(dir)) != nullptr) {
        ...
        if (filename.size() >= extension.size() && 
            ...
            if (stat(filename.c_str(), &fileStat) == 0) {
                std::cout << "Arquivo encontrado: " << filename 
                          << " | Tamanho: " << fileStat.st_size << " bytes" << std::endl;  // Imprime o nome e tamanho do arquivo
                found = true;  // Marca que um arquivo foi encontrado
            } else {
                std::cerr << "Não foi possível obter informações do arquivo: " << filename << std::endl;
            }
        }
    }
